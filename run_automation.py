# run_automation.py (9th Amendment - Full Version)
# Includes signal_time_utc in JSON for precise backtesting
...
MIN_PROFIT_MARGIN = 2.0           # min % on margin
# PROFIT_CEILING_MARGIN removed in 9th Amendment
...
# ============== ANALYZE FUNCTION (9th Amendment) ==============
def analyze_data(symbol, data5m, market_trend):
    """
    9th Amendment:
    - Separate scoring for Buy vs Sell
    - Confidence-based promotion
    - No profit ceiling
    - Adds signal_time_utc
    """
    ...
    raw_profit_pct = abs((tp - current_price) / current_price * 100)
    est_profit_margin = raw_profit_pct * LEVERAGE_FOR_PROFIT_EVAL
    passes_min_profit = est_profit_margin >= MIN_PROFIT_MARGIN

    # Confidence calculation
    base_score = buy_score if initial_signal == "Buy" else sell_score if initial_signal == "Sell" else 0
    base_component = (base_score / 100) * 0.4
    confluence_component = (num_conf / 3) * 0.4
    veto_component = (int(passes_min_profit) + int(passes_vol_buy if initial_signal == "Buy" else passes_vol_sell)) / 2 * 0.2
